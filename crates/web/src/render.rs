...
fn draw_header(ctx: &CanvasRenderingContext2d, workflow: &Workflow) {
    ...
    ctx.set_font(&format!("bold {}px {}", theme::FONT_SIZE_HEADER, theme::FONT_FAMILY));
    ctx.set_fill_style_str(theme::COLOR_HEADER_TEXT);
    ctx.fill_text(&workflow.name, x, y).ok();

    ctx.set_font(&format!("{}px {}", theme::FONT_SIZE_HEADER, theme::FONT_FAMILY));
    ctx.set_fill_style_str(theme::COLOR_HEADER_TRIGGER);
    let name_width = ctx.measure_text(&workflow.name).map(|m| m.width()).unwrap_or(60.0);
    ctx.fill_text(&workflow.trigger, x + name_width + 12.0, y).ok();
}

fn draw_edge(ctx: &CanvasRenderingContext2d, from: &NodeLayout, to: &NodeLayout, highlighted: bool) {
    ...
    ctx.arc(x1, y1, theme::JUNCTION_DOT_RADIUS, 0.0, 2.0 * PI).ok();
    ctx.fill();

    // Junction dot at target
    ctx.begin_path();
    ctx.arc(x2, y2, theme::JUNCTION_DOT_RADIUS, 0.0, 2.0 * PI).ok();
    ctx.fill();
}
...
fn draw_node(
    ...
    ctx.set_font(&format!("600 {}px {}", theme::FONT_SIZE_NAME, theme::FONT_FAMILY));
    ...
    if let Some(dur_text) = duration_str {
        ctx.set_font(&format!("{}px {}", theme::FONT_SIZE_DURATION, theme::FONT_FAMILY));
        ...
        let dur_width = ctx.measure_text(&dur_text).map(|m| m.width()).unwrap_or(30.0);
...
fn draw_octicon(ctx: &CanvasRenderingContext2d, cx: f64, cy: f64, radius: f64, path_data: &str, color: &str) {
...
fn draw_status_icon(
    ...
    match status {
        ...
        JobStatus::Success => {
            ...
            draw_octicon(ctx, x, y, r, OCTICON_CHECK_CIRCLE_FILL, theme::COLOR_SUCCESS);
        }
        JobStatus::Failure => {
